...
## Build 9 Downloads permission and update identity

Two consecutive PNG-to-JPEG actions were invoked through the actual Finder Convert menu in a dedicated Downloads fixture folder on the unchanged installed build 9. Both used separate workers and completed without another folder-access prompt. Both outputs were 96×64, the second received a collision suffix, and the source SHA-256 remained unchanged. Worker lifecycle logs reported zero visible windows and accessory activation throughout both jobs. Only generated test files were used.
